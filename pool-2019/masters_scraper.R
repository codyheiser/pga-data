source('~/git/codyr/utilityfunctions.r',chdir = T)
...
years <- c(1997:2018) # years to query
...
# import 2018 field
field.2019<- read.csv('pool-2019/mastersfield2019.csv')
field.2019$First <- vectorsplit(field.2019$name,'\\ ',keep = 1)
field.2019$Last <- vectorsplit(field.2019$name,'\\ ',keep = 2)

left_join(field.2019,CompleteData,by = c('First','Last')) %>%
  select(-name.x) %>%
...
ratio <- merge(ratio,field.2019[c('First','Last','group')],by = c('First','Last'),all.x = T, all.y = F)
